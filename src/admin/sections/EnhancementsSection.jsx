...
const EnhancementsSection = () => {
  ...
  return (
    <>
      <Card
        title={__('LQIP placeholders', 'oxpulse-imager')}
        description={__('Low-quality image placeholders — tiny blurred previews via imgproxy that reduce Cumulative Layout Shift (CLS). Falls back to inline SVG when imgproxy is unreachable.', 'oxpulse-imager')}
      >
        <ToggleField
          name="lqip_enabled"
      ...
      <Card
        title={__('DPR-aware srcset', 'oxpulse-imager')}
        description={__('For img tags with width but no srcset, generates 1x/2x/3x x-descriptor variants via imgproxy dpr: option. Images that already have w-descriptor srcset are left alone.', 'oxpulse-imager')}
      >
        <ToggleField
          name="dpr_enabled"
      ...
      <Card
        title={__('Watermark', 'oxpulse-imager')}
        description={__('Applies imgproxy native watermark (wm: option). The watermark image is configured server-side via IMGPROXY_WATERMARK_PATH/URL — this setting controls placement only.', 'oxpulse-imager')}
      >
        <ToggleField
